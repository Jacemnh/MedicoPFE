...
import {
  ...
  X, CalendarCheck, User,
  MapPin, Activity, Briefcase, Settings, Euro, CreditCard
} from 'lucide-react';
...
const ListePatients = () => {
  ...
  const fetchPatients = async () => {
    setLoading(true);
    try {
      const response = await axios.get('/pro/patients');
  ...
  const filtered = patients.filter(p => {
    ...
    // Filter logic can be refined if we have more status data
    return matchSearch;
  });
  ...
  return (
    <div className="lp-layout">
      ...
      <main className="lp-main">
        ...
        {/* Header */}
        <div className="lp-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div className="lp-header-badge">
              <Users size={16} />
              <span>Gestion des patients</span>
            </div>
            <h1 className="lp-title">Mes patients</h1>
            <p className="lp-subtitle">Gérez votre patientèle</p>
          </div>
          ...
          <div className="pro-header-actions" style={{ gap: '5px', display: 'flex', alignItems: 'center' }}>
            ...
            <Link to="/professionnel/profil" className="pro-user-info" style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 9px', border: '1px solid #ffffffff', borderRadius: '50px', textDecoration: 'none', cursor: 'pointer', background: '#ffffff', transition: 'all 0.2s' }}>
              <User size={18} color="#030303ff" />
              <span style={{ fontSize: '0.95rem', fontWeight: 500, color: '#080808ff' }}>
                Dr. {user?.nom}
              </span>
            </Link>
          </div>
        ...
        <div className="lp-stats">
          ...
          <div className="lp-stat-card" style={{ '--c': '#3B82F6' }}>
            <div className="lp-sc-icon" style={{ background: 'rgba(59,130,246,0.15)', color: '#3B82F6' }}>
              <CalendarCheck size={22} />
            </div>
            <div className="lp-sc-body">
              <span className="lp-sc-value">{patients.filter(p => p.prochainRDV).length}</span>
              <span className="lp-sc-label">RDV à venir</span>
            </div>
          </div>
          <div className="lp-stat-card" style={{ '--c': '#10B981' }}>
            ...
            <div className="lp-sc-body">
              ...
              <span className="lp-sc-label">Consultations totales</span>
            </div>
          </div>
        </div>
                ...
                <tbody>
                  {filtered.map((p, i) => (
                    <tr key={p.id} style={{ '--delay': `${i * 0.05}s` }} className="lp-tr-animated">
                      ...
                      <td>
                        <span className="lp-age-badge">{calculateAge(p.date_naissance)} ans</span>
                      </td>
                      <td>
              ...
              <div className="lp-modal-header">
                ...
                <div className="lp-mh-info">
                  ...
                  <div className="lp-mh-meta">
                    ...
                    <span><Calendar size={14} /> Né(e) le {selectedPatient.date_naissance ? new Date(selectedPatient.date_naissance).toLocaleDateString('fr-FR') : '—'}</span>
                  </div>
                </div>
              </div>
